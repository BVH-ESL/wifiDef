...
const uint8_t MCP_NUM_CHANNELS = 2;
...
void setup() {
  ...
  Serial.println();
  if(!rtc.isset()){
    rtc.adjust(DateTime(2016, 8, 17, 14, 0, 0));
    rtc.init();
  }
  //set sqwmode
  ...
  sprintf( fileNameSD, "log@%02d%02d-%02d%02d.txt",
       now.month(),
       now.day(),
       now.hour(),
       now.minute()
  ...
  dataFileSD.println("MM/DD/YYYY HH:mm:ss:msms,ch01,ch02");                      //write headder
...
void loop(){
  if(!isDone){
    if (counter_micro_second % 250 == 0)
    {
      ...
      sprintf( bufSD, "%02u/%02u/%04u %02u:%02u:%02u:%04d,%04d,%04d",
           timeNow.month(),
           timeNow.day(),
           timeNow.year(),
           timeNow.hour(),
