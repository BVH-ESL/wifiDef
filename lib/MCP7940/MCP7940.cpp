...
#define RTC_OSC     0x08
#define RTC_PWRDMIN       0x18
#define RTC_PWRDHR        0x19
#define RTC_PWRUMIN       0x1C
#define RTC_PWRUHR        0x1D
...
void RTC_MCP7940::init(){
  Wire.beginTransmission(RTC_ADD);
  Wire.write(RTC_SEC);
  Wire.write(0x80);
  Wire.endTransmission();
  Wire.beginTransmission(RTC_ADD);
  Wire.write(RTC_WKDAY);
  Wire.write(0x28);
  Wire.endTransmission();
}
...
void RTC_MCP7940::adjust(const DateTime& dt) {	//change date and time registers based on user input
  ...
  Wire.write(bin2bcd(0x08)); //enable RTC battery backup function
...
void RTC_MCP7940::setAlarm0Sec(uint8_t value){
  Wire.beginTransmission(RTC_ADD);
  Wire.write((RTC_ALARM));
  Wire.write((value));
  Wire.endTransmission();
...
uint8_t RTC_MCP7940::readSqwPinMode() {
  int mode;
  ...
  Wire.requestFrom((uint8_t)RTC_ADD, (uint8_t)7);
  mode = Wire.read();
...
void RTC_MCP7940::writeSqwPinMode(SqwPinMode mode) {
  Wire.beginTransmission(RTC_ADD);
  Wire.write(RTC_CONREG);
  Wire.write(mode);
  Wire.endTransmission();
